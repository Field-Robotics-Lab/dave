...
void UnderwaterCurrentPlugin::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
  ...
  GZ_ASSERT(this->sdf->HasElement("depth_dependent_current"),
    "Depth dependent current configuration not available");
  ...
  if (currentVelocityParams->HasElement("database"))
    ...
    this->databaseFilePath = "world://ocean_current_vs_depth.csv";

  GZ_ASSERT(!this->databaseFilePath.empty(),
    "Empty database file path");
  gzmsg << "\tWARNING: Database should be in NED frame"
    << std::endl;

  // Read database
  std::ifstream csvFile; std::string line;
  csvFile.open(this->databaseFilePath.c_str());
  ...
  while (getline(csvFile,line))
  {
      ...
      read.X() = row[0]; read.Y() = row[0]; read.Z() = row[0];
  ...
  if (currentVelocityParams->HasElement("vehicle_depth_topic"))
    this->vehicleDepthTopic =
      currentVelocityParams->Get<std::string>("vehicle_depth_topic");
  else
  ...
  // Advertise the vehicle depth topic
  this->publishers[this->vehicleDepthTopic] =
    this->node->Advertise<ignition::msgs::Double>(
    this->ns + "/" + this->vehicleDepthTopic);

  gzmsg << "Base vehicle depth topic name: " <<
    this->ns + "/" + this->vehicleDepthTopic << std::endl;

  // Subscribe vehicle depth topic  
  this->vehicleDepthSub =
    this->node->Subscribe(
    this->ns + "/" + this->vehicleDepthTopic, &UnderwaterCurrentPlugin::subsribeVehicleDepth, this);
...
void UnderwaterCurrentPlugin::Update(const common::UpdateInfo & /** _info */)
{
  ...
  // this->ApplyDatabase();
  gzmsg << this->vehicleDepth << std::endl;
...
void UnderwaterCurrentPlugin::ApplyDatabase()
{
  ...
  // //Interpolate
  // this->currentVelocity = this->currentVelocity + velocity;
}

/////////////////////////////////////////////////
void UnderwaterCurrentPlugin::subsribeVehicleDepth(ConstDoublePtr &_msg)
{
  this->vehicleDepth = _msg->value();
}
