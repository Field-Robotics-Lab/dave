...
#include <uuv_dave_world_plugins/GaussMarkovProcess.hh>
#include <sdf/sdf.hh>
...
{ 
  ...
  typedef const boost::shared_ptr<const dave_gazebo_plugins_msgs::msgs::Double> ConstDoublePtr;
  ...
  class UnderwaterCurrentPlugin : public WorldPlugin
  {
    ...
    /// \brief Apply transient current from database (NED)
    protected: void ApplyDatabase();

    /// \brief Save subscribed vehicle depth
    protected: void subsribeVehicleDepth(ConstDoublePtr &_msg);
    ...
    /// \brief Subscriber for vehicle base depth
    private: transport::SubscriberPtr vehicleDepthSub; 

    /// \brief Vehicle base depth topic
    protected: std::string vehicleDepthTopic;

    /// \brief Vehicle base depth
